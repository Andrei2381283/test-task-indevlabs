const findMissingNum = (arr) => {
  ...
  for(const num of arr) {
    ...
    if(typeof num !== 'number') throw 'Array contains non number value'; // Throw error because non number values can break function
    if(num < minNum) minNum = num; // Find minimum number in array for ignore
  }
  for(const num of arr) {
    if(num === minNum) continue; // Ignore minimum number
    if(!obj[num - 2] && num > minNum + 2) throw 'Multiple answers'; // Throw error because array contains multiple missing values
